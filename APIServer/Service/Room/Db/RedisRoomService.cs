...
public class RedisRoomDbService : IRoomDbService
{
    ...
    //readonly String SCRIPT_ROOM_ENTER = "if redis.call('EXISTS', @KEY0) == 1 then return 103 end local roomId = tonumber(@ARGV2) if @ARGV2 == '-1' then roomId = math.random(0, 9999) while redis.call('EXISTS', 'room:' .. roomId .. ':title') == 1 do roomId = math.random(0, 9999) end end local title = redis.call('GET', @KEY1) if title == false or title == nil then redis.call('DEL', @KEY2, @KEY3, @KEY4, @KEY5) if tonumber(redis.call('GET', 'room:count')) >= 49 then return 107 end redis.call('INCR', 'room:count') local title2 = 'Room' .. roomId redis.call('SET', @KEY1, title2) redis.call('SET', @KEY0, roomId) redis.call('ZADD', @KEY5, @ARGV0, @ARGV1) redis.call('ZADD', @KEY2, @ARGV0, @ARGV1) return '\tI ' .. roomId .. '\tT ' .. title2 .. '\tU ' .. @ARGV1 .. '\tH ' .. @ARGV1 .. '\tu ' .. @ARGV0 .. '\th ' .. @ARGV0 end local userIds = redis.call('ZRANGE', @KEY2, 0, -1) if #userIds >= 4 then return 102 end redis.call('SET', @KEY0, roomId) redis.call('ZADD', @KEY2, @ARGV0, @ARGV1) local alluserIds = {} local allnicknames = {} local users = redis.call('ZRANGE', @KEY2, 0, -1, 'WITHSCORES') for i = 1, #users, 2 do table.insert(alluserIds, tonumber(users[i + 1])) table.insert(allnicknames, users[i]) end local anicknames = {} local bnicknames = {} local readys_a = redis.call('ZRANGE', @KEY3, 0, -1, 'WITHSCORES') for i = 1, #readys_a, 2 do table.insert(anicknames, readys_a[i]) end local readys_b = redis.call('ZRANGE', @KEY4, 0, -1, 'WITHSCORES') for i = 1, #readys_b, 2 do table.insert(bnicknames, readys_b[i]) end local host = redis.call('ZRANGE', 'room:' .. roomId.. ':host', 0, -1, 'WITHSCORES') return '\tI ' .. roomId .. '\tT ' .. title .. '\tU ' .. table.concat(allnicknames, ' ') .. '\tH '  .. host[1] .. '\tA ' .. table.concat(anicknames, ' ') .. '\tB ' .. table.concat(bnicknames, ' ') .. '\tu ' .. table.concat(alluserIds, ' ') .. '\th ' .. host[2]";
    readonly String SCRIPT_ROOM_ENTER = "local roomId = tonumber(@ARGV2) if @ARGV2 == '-1' then roomId = math.random(0, 9999) while redis.call('EXISTS', 'room:' .. roomId .. ':title') == 1 do roomId = math.random(0, 9999) end end local titleKey = 'room:' .. roomId  .. ':title' local usersKey = 'room:' .. roomId  .. ':users' local readys_aKey = 'room:' .. roomId  .. ':readys_a' local readys_bKey = 'room:' .. roomId  .. ':readys_b' local hostKey = 'room:' .. roomId  .. ':host' if redis.call('EXISTS', @KEY0) == 1 then return 103 end local title = redis.call('GET', titleKey) if title == false or title == nil then redis.call('DEL', usersKey, readys_aKey, readys_bKey, hostKey) if tonumber(redis.call('GET', 'room:count')) >= 49 then return 107 end redis.call('INCR', 'room:count') local title2 = 'Room' .. roomId redis.call('SET', titleKey, title2) redis.call('SET', @KEY0, roomId) redis.call('ZADD', hostKey, @ARGV0, @ARGV1) redis.call('ZADD', usersKey, @ARGV0, @ARGV1) return '\tI ' .. roomId .. '\tT ' .. title2 .. '\tU ' .. @ARGV1 .. '\tH ' .. @ARGV1 .. '\tu ' .. @ARGV0 .. '\th ' .. @ARGV0 end local userIds = redis.call('ZRANGE', usersKey, 0, -1) if #userIds >= 4 then return 102 end redis.call('SET', @KEY0, roomId) redis.call('ZADD', usersKey, @ARGV0, @ARGV1) local alluserIds = {} local allnicknames = {} local users = redis.call('ZRANGE', usersKey, 0, -1, 'WITHSCORES') for i = 1, #users, 2 do table.insert(alluserIds, tonumber(users[i + 1])) table.insert(allnicknames, users[i]) end local anicknames = {} local bnicknames = {} local readys_a = redis.call('ZRANGE', readys_aKey, 0, -1, 'WITHSCORES') for i = 1, #readys_a, 2 do table.insert(anicknames, readys_a[i]) end local readys_b = redis.call('ZRANGE', readys_bKey, 0, -1, 'WITHSCORES') for i = 1, #readys_b, 2 do table.insert(bnicknames, readys_b[i]) end local host = redis.call('ZRANGE', hostKey, 0, -1, 'WITHSCORES') return '\tI ' .. roomId .. '\tT ' .. title .. '\tU ' .. table.concat(allnicknames, ' ') .. '\tH ' .. host[1] .. '\tA ' .. table.concat(anicknames, ' ') .. '\tB ' .. table.concat(bnicknames, ' ') .. '\tu ' .. table.concat(alluserIds, ' ') .. '\th ' .. host[2]";
